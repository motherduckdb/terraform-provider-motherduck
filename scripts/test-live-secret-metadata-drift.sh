...
if [[ "${drift_plan_exit}" -ne 2 ]]; then
  echo "Expected Terraform to detect out-of-band secret scope drift with exit code 2, got ${drift_plan_exit}" >&2
  exit 1
fi
...
if [[ "${repair_plan_exit}" -ne 0 ]]; then
  if [[ "${repair_plan_exit}" -eq 2 ]]; then
    echo "Expected no-op plan after repairing secret scope drift, but Terraform reported changes" >&2
  fi
