...
cleanup() {
  ...
  if [[ "${KEEP_LIVE_FIXTURE}" != "1" ]]; then
    live_drop_share "${share_alpha}" || destroy_status=$?
    live_drop_share "${share_beta}" || destroy_status=$?
    live_drop_database "${database_alpha}" || destroy_status=$?
    live_drop_database "${database_beta}" || destroy_status=$?
  fi
...
expect_noop_plan "Expected no-op plan after SQL-only blueprint apply, but Terraform reported changes" \
