...
work_dir="${ROOT_DIR}/test-results/live-table-unmanaged-view-${RUN_ID}"
if [[ -e "${work_dir}" ]]; then
  echo "Refusing to reuse existing test directory: ${work_dir}" >&2
  exit 1
fi
mkdir -p "${work_dir}"
cp "${ROOT_DIR}/test-fixtures/live-table-unmanaged-view/main.tf" "${work_dir}/main.tf"
...
database_name="tf_unmanaged_view_${RUN_ID//-/_}"
schema_name=""
table_name=""
unmanaged_view_name="unmanaged_facts_v"
...
echo "==> Live unmanaged dependent view smoke (${RUN_ID})"
write_vars true
...
go run "${ROOT_DIR}/internal/dev/mdexec" -database "${database_name}" -sql "CREATE VIEW \"${database_name}\".\"${schema_name}\".\"${unmanaged_view_name}\" AS SELECT label FROM \"${database_name}\".\"${schema_name}\".\"${table_name}\""
go run "${ROOT_DIR}/internal/dev/mdexec" -database "${database_name}" -scalar "SELECT count(*)::VARCHAR FROM \"${database_name}\".\"${schema_name}\".\"${unmanaged_view_name}\"" >/dev/null

write_vars false
TF_CLI_CONFIG_FILE="${cli_config}" "${TERRAFORM_BIN}" -chdir="${work_dir}" apply -auto-approve -input=false

set +e
view_query_output="$(go run "${ROOT_DIR}/internal/dev/mdexec" -database "${database_name}" -scalar "SELECT count(*)::VARCHAR FROM \"${database_name}\".\"${schema_name}\".\"${unmanaged_view_name}\"" 2>&1)"
view_query_exit=$?
set -e

if [[ "${view_query_exit}" -eq 0 ]]; then
  echo "Expected unmanaged view to fail after managed table replacement removed the label column" >&2
  exit 1
fi
printf '%s\n' "${view_query_output}" | grep -qi "label" || {
  echo "Expected unmanaged view failure to mention the removed label column" >&2
  printf '%s\n' "${view_query_output}" >&2
  exit 1
}

expect_noop_plan "Expected no-op plan after managed table replacement, but Terraform reported changes" \
