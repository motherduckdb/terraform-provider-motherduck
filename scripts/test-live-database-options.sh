...
work_dir="${ROOT_DIR}/test-results/live-database-options-${RUN_ID}"
if [[ -e "${work_dir}" ]]; then
  echo "Refusing to reuse existing test directory: ${work_dir}" >&2
  exit 1
fi
mkdir -p "${work_dir}"
cp "${ROOT_DIR}/test-fixtures/live-database-options/main.tf" "${work_dir}/main.tf"
...
suffix="${RUN_ID//-/_}"
default_database_name="tf_db_options_default_${suffix}"
transient_database_name="tf_db_options_transient_${suffix}"
...
cleanup() {
  ...
  if [[ "${KEEP_LIVE_FIXTURE}" != "1" ]]; then
    live_drop_database "${transient_database_name}" || destroy_status=$?
    live_drop_database "${default_database_name}" || destroy_status=$?
  fi
  return "${destroy_status}"
}
trap live_cleanup_on_exit EXIT

echo "==> Live database options smoke (${RUN_ID})"
...
if [[ "$(TF_CLI_CONFIG_FILE="${cli_config}" "${TERRAFORM_BIN}" -chdir="${work_dir}" output -raw default_database_type)" != "default" ]]; then
  echo "Expected explicit default database_type to read back as default" >&2
  exit 1
fi
if [[ "$(TF_CLI_CONFIG_FILE="${cli_config}" "${TERRAFORM_BIN}" -chdir="${work_dir}" output -raw transient_database_is_transient)" != "true" ]]; then
  echo "Expected transient database to read back transient=true" >&2
  exit 1
fi

expect_noop_plan "Expected no-op plan after database options apply, but Terraform reported changes" \
