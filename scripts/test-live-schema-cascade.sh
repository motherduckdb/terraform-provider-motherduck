...
work_dir="${ROOT_DIR}/test-results/live-schema-cascade-${RUN_ID}"
if [[ -e "${work_dir}" ]]; then
  echo "Refusing to reuse existing test directory: ${work_dir}" >&2
  exit 1
fi
mkdir -p "${work_dir}"
cp "${ROOT_DIR}/test-fixtures/live-schema-cascade/main.tf" "${work_dir}/main.tf"
...
run_id = "${RUN_ID}"
schema_cascade_on_delete = ${cascade}
HCL
}

database_name="tf_schema_cascade_${RUN_ID//-/_}"
schema_name=""
...
echo "==> Live schema cascade smoke (${RUN_ID})"
write_vars false
...
go run "${ROOT_DIR}/internal/dev/mdexec" -database "${database_name}" -sql "CREATE TABLE \"${database_name}\".\"${schema_name}\".\"unmanaged_facts\" (id INTEGER)"

set +e
TF_CLI_CONFIG_FILE="${cli_config}" "${TERRAFORM_BIN}" -chdir="${work_dir}" destroy -auto-approve -input=false
restrict_destroy_exit=$?
set -e

if [[ "${restrict_destroy_exit}" -eq 0 ]]; then
  echo "Expected restrictive schema destroy to fail while unmanaged objects exist" >&2
  exit 1
fi
