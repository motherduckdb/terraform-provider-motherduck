...
plan_module() {
  ...
  MOTHERDUCK_TOKEN="offline-sql-token" MOTHERDUCK_ADMIN_TOKEN="offline-admin-token" \
    ...
    plan -refresh=false -input=false -no-color -out="${plan}" >/dev/null
...
writer_retire_plan="$(plan_module "${writer_retire_dir}")"
jq -e '([.resource_changes[].address] | any(. == "module.example.motherduck_access_token.writer_legacy[0]") | not) and (.planned_values.outputs.writer_token.value == null) and (.planned_values.outputs.writer_rotation_tokens.sensitive == true)' <<<"${writer_retire_plan}" >/dev/null
