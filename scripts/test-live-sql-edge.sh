...
cleanup() {
  ...
  if [[ "${KEEP_LIVE_FIXTURE}" != "1" ]]; then
    live_unname_snapshot "${database_name}" "${initial_snapshot_name}"
    live_unname_snapshot "${database_name}" "${updated_snapshot_name}"
...
if [[ "${plan_exit}" -ne 0 ]]; then
  if [[ "${plan_exit}" -eq 2 ]]; then
    echo "Expected no-op plan after update, but Terraform reported changes" >&2
  fi
  exit "${plan_exit}"
fi
