---
page_title: "Hypertenancy blueprint"
subcategory: "Reusable blueprints"
description: |-
  Give every tenant an isolated database, restricted share, reader account, and read-scaling token.
---
...
- `motherduck_access_token` (a one-hour setup token, a read-scaling token, and one token per rotation generation)
- `motherduck_share`
- `motherduck_share_grant`

The example implementation is in `examples/blueprints/hypertenancy`.

This blueprint includes REST-backed service-account and access-token resources. It requires `MOTHERDUCK_TOKEN` for SQL resources and an organization-admin `MOTHERDUCK_ADMIN_TOKEN` for reader service accounts, Duckling configuration, and token creation. Share grants run over SQL as the share owner.
...
2. Move `module.hypertenancy.reader_rotation_tokens["acme/2026_10"]` (keyed by `<tenant_id>/<generation>`) into each backend secret and verify new connections.
