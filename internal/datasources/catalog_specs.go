...
func rowSpecs() []rowSpec {
	return []rowSpec{
		...
		{name: "attached_databases", description: "Reads databases attached to the current MotherDuck SQL session. Rows are sorted by alias and name.", requiredFunction: "md_attached_databases", postProcess: sortRowsByKeys(rowSortKey{field: "alias"}, rowSortKey{field: "name"}), build: func(m rowsModel) (string, error) {
		...
		{name: "files", description: "Lists files for an object-storage path through MotherDuck SQL. Rows are sorted by path. Set `limit` and `offset` to page through large listings. Paging orders the listing by all of its columns, so pages are stable, and rows within each page are then sorted.", requiredFunction: "md_list_files", attrs: []string{"path", "limit", "offset"}, requiredAttrs: []string{"path"}, postProcess: sortRowsByKeys(rowSortKey{field: "path"}, rowSortKey{field: "name"}, rowSortKey{field: "key"}), build: func(m rowsModel) (string, error) {
			...
			return appendOrderedRowLimitOffset("SELECT * FROM md_list_files("+sqlbuild.StringLiteral(m.Path.ValueString())+")", "ALL", m), nil
		}},
		...
		{name: "dives", description: "Lists MotherDuck Dives available to the current account. Rows in the returned page are sorted by Dive ID.", requiredFunction: "md_list_dives", attrs: []string{"limit", "offset", "include_org_shares"}, postProcess: sortRowsByKeys(rowSortKey{field: "id"}), build: func(m rowsModel) (string, error) {
		...
		{name: "dive_versions", description: "Lists versions for one MotherDuck Dive, newest version first.", requiredFunction: "md_list_dive_versions", attrs: []string{"dive_id"}, requiredAttrs: []string{"dive_id"}, postProcess: sortRowsByKeys(newestVersionFirst()...), build: func(m rowsModel) (string, error) {
			if m.DiveID.IsNull() {
				return "", fmt.Errorf("dive_id is required")
			}
		...
		{name: "guide_versions", description: "Lists version history for one MotherDuck Guide without returning content. Rows in the returned page are sorted newest version first.", requiredFunction: "md_list_guide_versions", attrs: []string{"guide_id", "limit", "offset"}, requiredAttrs: []string{"guide_id"}, postProcess: sortRowsByKeys(newestVersionFirst()...), build: func(m rowsModel) (string, error) {
			if m.GuideID.IsNull() {
				return "", fmt.Errorf("guide_id is required")
			}
...
func guideReferenceFilterArg(model rowsModel) (string, bool, error) {
	...
	reference := guideref.Reference{
		Type: model.ReferenceType, URL: model.ReferenceURL, Schema: model.ReferenceSchema, Table: model.ReferenceTable,
		Column: model.ReferenceColumn, View: model.ReferenceView, Macro: model.ReferenceMacro, UUID: model.ReferenceUUID,
	}
	refType := model.ReferenceType.ValueString()
	switch guideref.Validate(reference) {
	case guideref.Valid:
		return guideref.StructLiteral(reference, types.StringNull()), true, nil
...
// newestVersionFirst orders version listings by their version number. The
// candidate columns cover the naming used across Dive, Flight, and Guide
// version functions.
func newestVersionFirst() []rowSortKey {
	return []rowSortKey{
		{field: "version_number", descending: true},
		{field: "version", descending: true},
	}
}

// appendOrderedRowLimitOffset applies SQL paging to a table function that has
// no paging arguments. Paging needs a total order, so the query is ordered
// only when limit or offset is set, which keeps the unpaged query unchanged.
func appendOrderedRowLimitOffset(query, orderBy string, model rowsModel) string {
	limited := !model.Limit.IsNull() && !model.Limit.IsUnknown()
	offset := !model.Offset.IsNull() && !model.Offset.IsUnknown()
	if !limited && !offset {
		return query
	}
	return appendRowLimitOffset(query+" ORDER BY "+orderBy, model)
}
