...
description: |-
  Manage role membership and share grants with Terraform, and audit access for drift.
---

# Manage roles and share access

Custom roles require a MotherDuck Business plan or an active free trial, and permission to
...
Terraform checks grants in its state. A plan with no changes does not prove
...
The [example](https://github.com/motherduckdb/terraform-provider-motherduck/blob/v0.2.13/examples/access-control/README.md) maps each team to a role
...
Copy the [example](https://github.com/motherduckdb/terraform-provider-motherduck/blob/v0.2.13/examples/access-control/README.md) into a Terraform
root. Set `teams` in `terraform.tfvars` so later `terraform import` commands
...
[read-only share audit example](https://github.com/motherduckdb/terraform-provider-motherduck/blob/v0.2.13/examples/share-access-audit/README.md).
...
The [role access audit example](https://github.com/motherduckdb/terraform-provider-motherduck/blob/v0.2.13/examples/role-access-audit/README.md)
performs the same comparison for role membership.
...
1. Read [motherduck_role_members](../data-sources/role_members.md) for each role
   to check its user and role members.
2. Read [motherduck_roles_for_role](../data-sources/roles_for_role.md) to check
   role inheritance, including platform roles.
3. Read [motherduck_share_grants](../data-sources/share_grants.md) for each share
   to check who has a grant and whether access extends to the organization or
   anyone with the share URL.
4. Compare the results with the configuration. Import grants you intend to
   manage and revoke grants that should not exist.
...
Reading these data sources does not enforce the configuration. The comparison
is what identifies grants outside Terraform's state. Applying a plan restores
managed grants but does not revoke grants Terraform does not own.
...
- A share's `include_pattern` selects the tables it exposes. Audiences that need
  different tables need different shares. Filtered shares need a Business plan
  or an active free trial and work only on standard MotherDuck databases, not
  DuckLake or Iceberg databases.
