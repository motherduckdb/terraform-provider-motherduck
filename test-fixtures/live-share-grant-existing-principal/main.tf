...
variable "grantee_username" {
  type      = string
  sensitive = true

  validation {
    condition     = length(trimspace(var.grantee_username)) > 0
    error_message = "grantee_username must not be empty."
  }
}

locals {
  suffix      = replace(var.run_id, "-", "_")
  database    = "tf_grant_existing_${local.suffix}"
  share       = "tf_grant_existing_share_${local.suffix}"
  schema_name = "app"
}
